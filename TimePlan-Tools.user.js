...
// @version      1.15.10
// @description  Operational Interactive Board with PickUpPoint below Click & Collect and fixed shift-ended menus
...
(function () {
    ...
    const BOARD_AREAS = [
        ...
        { group: 'PickUpPoint', id: 'pickup-point', areas: [
            { id: 'pup-floor', name: 'Floor' },
            { id: 'pup-mh', name: 'MH' },
            { id: 'pup-reachtruck', name: 'Reachtruck' },
            { id: 'pup-tpl', name: 'TPL' }
        ]}
    ];
...
<script>window.onload=()=>setTimeout(()=>window.print(),120)<\/script>
    ...
    async function buildInteractiveBoardHTML() {
        ...
        const stateJSON = safeJSONStringify({ assignments: data.assignments, notPresent: data.notPresent, supportDestinations: {}, supportTimes: {}, lastUpdated: new Date().toISOString() });
...
<script>
const DATA=${dataJSON};
let state={assignments:{},notPresent:{},supportDestinations:{},supportTimes:{},lastUpdated:null};
...
function shiftStatus(w){if(DATA.date!==localISODate())return null;const now=new Date();const m=now.getHours()*60+now.getMinutes(),end=timeMinutes(w.end);/* Shift-end status is visual/capacity-only. Never change assignments here: an ended coworker stays in the flow/area where they finished. */if(m>=end)return{ended:true,text:'SHIFT ENDED'};if(end-m<=30)return{soon:true,text:'OFF '+w.end};return null}
function isWorkerActive(w){return !notPresent[w.key]&&!((shiftStatus(w)||{}).ended)}
function workerHTML(w){const np=!!notPresent[w.key];const adjusted=!!workerOverrides[w.key]&&!w.isExternal;const ss=np?null:shiftStatus(w);const support=assignment(w)==='support'?(supportDestinations[w.key]||'Other department'):'';const supportTime=support?(supportTimes[w.key]||null):null;const supportLabel=support?(support+(supportTime?' \u00B7 '+supportTime.start+'-'+supportTime.end:'')):'';return '<div class="worker'+(np?' not-present':'')+(ss&&ss.ended?' shift-ended':'')+'" draggable="'+(!np)+'" data-worker-key="'+esc(w.key)+'"><span class="function-bar" style="background:'+(np?'#999':w.functionBar)+'"></span><div class="worker-name"><span>'+esc(w.name)+'</span>'+(w.isExternal?'<span class="badge" style="background:#666;color:#fff">EXT</span>':badgesHTML(w))+(adjusted?'<span class="badge" style="background:#7A5AF8;color:#fff" title="Board time adjusted manually">TIME</span>':'')+(np?'<span class="badge" style="background:#666;color:#fff">ABSENCE</span>':'')+'</div><div class="worker-time"><span class="worker-start">'+esc(w.start)+'</span><span class="worker-arrow"> &rarr; </span><span class="worker-end">'+esc(w.end)+'</span></div>'+(w.isExternal?'<div style="margin-top:3px;color:#777;font-size:10px;font-weight:700">External help</div>':absenceHTML(w))+(support?'<div class="support-note">SUPPORT \u00B7 '+esc(supportLabel)+'</div>':'')+(ss?'<div class="shift-status '+(ss.ended?'shift-ended-label':'shift-soon')+'">'+ss.text+'</div>':'')+'<div class="magnet-menu"><button data-action="presence">'+(np?'Mark as available':'Mark as absent')+'</button><button data-action="time">Edit board time</button><button data-action="support">Assign to other department</button>'+(support?'<button data-action="edit-support">Edit support details</button>':'')+''+(adjusted?'<button data-action="restore-time">Restore TimePlan time</button>':'')+(w.isExternal?'<button data-action="edit">Edit external help</button><button data-action="remove" style="color:#A12622">Remove external help</button>':'')+'</div></div>'}
...
function saveHTML(){const html=currentPortableHTML();const blob=new Blob([html],{type:'text/html;charset=utf-8'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download='TimePlan-Interactive-Board-'+DATA.department+'-'+DATA.date+'.html';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);setDirty(false);document.getElementById('lastChange').textContent='Interactive Board exported';refreshHeaderTimes()}
async function quickShare(){const html=currentPortableHTML();const stamp=boardShareStamp();const filename='TimePlan-Interactive-Board-'+DATA.department+'-'+DATA.date+'.html';const subject='Daily Board Plan - '+stamp.date+' - '+stamp.time;const bodyText='Hi,\\n\\nPlease find the Interactive Daily Board Plan for '+stamp.date+'.\\nPlan shared at '+stamp.time+' local time.\\nDepartment '+DATA.department+'.\\n';const file=new File([html],filename,{type:'text/html'});try{if(navigator.share&&(!navigator.canShare||navigator.canShare({files:[file]}))){await navigator.share({title:subject,text:bodyText,files:[file]});setDirty(false);document.getElementById('lastChange').textContent='Handover shared';refreshHeaderTimes();return}}catch(err){if(err&&err.name==='AbortError')return}const blob=new Blob([html],{type:'text/html;charset=utf-8'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=filename;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);window.location.href='mailto:?subject='+encodeURIComponent(subject)+'&body='+encodeURIComponent(bodyText);setTimeout(()=>alert('Your browser cannot attach files directly to Outlook. The Interactive Board was downloaded and the email draft was opened. Please attach the downloaded HTML file.'),400)}
function preparePrintLayout(){document.querySelectorAll('.print-hidden').forEach(el=>el.classList.remove('print-hidden'));if(workersFor('unassigned').length===0)document.getElementById('unassignedWrapper').classList.add('print-hidden');if(workersFor('leadership').length===0)document.getElementById('leadership').classList.add('print-hidden');if(workersFor('support').length===0)document.getElementById('support').classList.add('print-hidden');document.querySelectorAll('#board .flow').forEach((flow,index)=>{let visible=0;flow.querySelectorAll('.dropzone').forEach(zone=>{const count=workersFor(zone.dataset.areaId).length;if(count===0)zone.classList.add('print-hidden');else visible+=count});if(visible===0)flow.classList.add('print-hidden')});const visibleFlows=[...document.querySelectorAll('#board .flow')].filter(el=>!el.classList.contains('print-hidden')).length;document.documentElement.style.setProperty('--print-columns',String(Math.max(1,Math.min(3,visibleFlows||1))))}
